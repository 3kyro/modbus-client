...
pFunctionSpec = describe "Parse a function code" $ do
  ...
  context "it correctly assigns functions codes" $ do
    it "at read input" $ testCSVParser pFunction "3;" `shouldBe` Right ReadInput
...
pValueSpec = describe "Parse a modbus value" $ do
  it "parses words" $ property $ \x -> Right (ModWord (Just x))
  ...
  it "ignores upper case words" $ property $ \x ->
    Right (ModWord (Just x)) == testCSVParser
      pValue
      (capitalizeLetter "word;" (fromIntegral x) ++ show x ++ ";")
  it "ignores upper case floats" $ property $ \x ->
    Right (ModFloat (Just x)) == testCSVParser
      pValue
      (capitalizeLetter "float;" (round x) ++ show x ++ ";")
  it "return Nothing on words" $ testCSVParser pValue "word;;" == Right
    (ModWord Nothing)
  it "return Nothing on floats" $ testCSVParser pValue "float;;" == Right
    (ModFloat Nothing)
...
prop_description_text s = valid s ==> Right (T.pack s) == testCSVParser
  pDescription
  (s ++ ";")
  where valid = all (`notElem` ";\n\r")
...
prop_non_numeric_function_code s =
  not (any isDigit s) && notElem ';' s ==> isLeft $ testCSVParser pFunction
                                                                  (s ++ ";")
...
prop_valid_datum
  :: String -> ModFunction -> Word16 -> ModType -> String -> Property
prop_valid_datum desc fun reg val com =
  validText desc
    &&  validText com
    ==> Right
          (ModData { description = T.pack desc
                   , function    = fun
                   , register    = reg
                   , value       = val
                   , comments    = T.pack com
...
instance Arbitrary ModFunction where
  arbitrary =
    elements [ReadInput, ReadMultHolding, WriteSingleHolding, WriteMultHolding]
...
makeModData desc fun reg val com = testCSVParser
  pModData
